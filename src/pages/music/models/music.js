...
* https://api.imjad.cn/cloudmusic/?type=lyric&id=259680&br=128000
* */
...
let songGetWithId = (id) => {
  return axios.get(`https://api.imjad.cn/cloudmusic/?type=song&id=${id}&br=128000`)
}

let songLyricGetWithId = (id) => {
...
let defaultList = [
  {
    ...
    "singer": "银临_Aki阿杰",
    "src": "/static/牵丝戏.mp3",
    "picUrl": "https://p2.music.126.net/J-5AzeDm8mOHmUXLWMy0OQ==/109951162822930517.jpg",
  ...
  },
  {
    ...
    "singer": "袁娅维",
    "src": "/static/一生等你.mp3",
    'picUrl': "https://p2.music.126.net/Zs33iCX9zVxJPQMB4p8spQ==/109951163462507650.jpg",
    ...
    * xhr({payload: {moduleName, params, resolve}}, {call, put, select}) {
      ...
      switch (moduleName) {
        ...
        case 'songGetWithId':
          ...
          break
        case 'songLyricGetWithId':
          return songLyricGetWithId(params)
          break
      }
      if (result.status === 0) {
        yield put({
